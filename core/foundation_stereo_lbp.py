...
try:
    ...
    autocast = partial(torch.amp.autocast, 'cuda', dtype=torch.bfloat16)
except:
    class autocast:
...
class FoundationStereo(nn.Module, huggingface_hub.PyTorchModelHubMixin):
    def __init__(self, args):
        ...
        self.lbp_encoder = LBPEncoder(args=self.args)
        self.modulater = BetaModulator(args=self.args,lbp_dim=self.lbp_encoder.num_neighbors)
    ...
    def forward(self, image1, image2, iters=12, flow_init=None, test_mode=False, low_memory=False, init_disp=None,psuedo_depth=None):
        """ Estimate disparity between pair of frames """
        B = len(image1)
        low_memory = low_memory or (self.args.get('low_memory', False))
        image1 = normalize_image(image1)
        image2 = normalize_image(image2)
        psuedo_depth = F.interpolate(psuedo_depth, scale_factor=0.25, mode='bilinear', antialias=True)
        depth_lbp = self.lbp_encoder(psuedo_depth)
        ...
        for itr in range(iters):
            ...
            with autocast(enabled=self.args.mixed_precision):
                ...
                modulation_weight = rescale_modulation(itr, iters,self.args.modulation_alg, self.args.modulation_ratio)  
