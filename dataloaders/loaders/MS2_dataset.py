...
class DataLoader_MS2(data.Dataset):
    ...
    def __getitem__(self, index):
        if isinstance(self.modality, list):
            ...
            results = self.data_getter(index, self.modality)
            results["extrinsics"] = self.get_extrinsic()
            return results
    ...
    def crawl_folders_depth(self, sampling_step, set_length, set_interval):
        ...
        for sensor in sensor_list:               
            ...
            for seq in self.seq_list: # iterate over each sequence
                ...
                calib = np.load(calib_path, allow_pickle=True).item()
                baseline_rgb = abs(calib['T_rgbR'][0].astype(np.float32))*0.001
                intrinsic_rgb = calib['K_rgbL'].astype(np.float32)
                ...
                for i in range(init_offset, len(img_list_left)-init_offset):
                    ...
                    sample = {'intrinsics': intrinsics, 
                              ...
                              'baseline' : baseline, 'extrinsics_L2R' : extrinsics_L2R,"baseline_rgb": baseline_rgb,"intrinsic_rgb": intrinsic_rgb
                             }
    ...
    def get_data_StereoMatching(self, index, modality):
        sample = self.samples[modality][index]
        # Read Psuedo-RGB image, its disparity and intrinsic
        # Apply transformation to get effective intrinsic matrix
        # Read extrinsic matrix  nir2rgb and nir2thr
        result = {}
        # read psueodo disparity and valid regions
        result["baseline_rgb"] = sample["baseline_rgb"]
        if self.data_split=="train" or self.data_split=="val":
            psuedo_disparity_dir = sample["tgt_img_left"].replace("sync_data","zero_shot_thermal").replace("/thr/img_left","").replace("png","npz")
            psuedo_disparity_dict = np.load(psuedo_disparity_dir)
            thr_psuedo_disparity = psuedo_disparity_dict["rgb_psuedo_disparity"] # Dictionary keys should be fixed
            valid_psuedo_disparity = psuedo_disparity_dict["valid_regions"]
            # read rgb intrinsic
        ...
        tgt_depth_gt    = load_as_float_depth(sample['tgt_depth_gt'])/256.0
        depth_inputs = [tgt_depth_gt, thr_psuedo_disparity] if (self.data_split == 'train' or self.data_split=="val") else [tgt_depth_gt]
        ...
        if self.data_split=="train"  or self.data_split=="val":
            f_old = (sample["intrinsics"][0,0]).item()
            f_new= (intrinsics[0,0]).item()
            result["thr_psuedo_disparity"] =depths[1]*(f_new/f_old)
            result["valid_psuedo_disparity"] = torch.from_numpy(valid_psuedo_disparity)
            #result["intrinsic_rgb"] = torch.from_numpy(sample['intrinsics'])
